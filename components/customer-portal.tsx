...
export function CustomerPortal() {
  const router = useRouter(); const { data, error } = useSWR<PortalData>("/api/portal/me", portalFetcher)
  async function logout() { await fetch("/api/portal/logout", { method: "POST" }); router.replace("/area-cliente/entrar"); router.refresh() }
  if (error) return <div className="mx-auto max-w-xl px-6 py-36 text-center"><p>Não foi possível abrir sua área.</p><Button className="mt-5" onClick={() => router.replace("/area-cliente/entrar")}>Entrar novamente</Button></div>
  ...
  return <section className="container mx-auto max-w-6xl px-6 pb-24 pt-32">
    ...
    <div className="mt-8 grid gap-5 lg:grid-cols-2">
      ...
      <div className="lg:col-span-2"><PortalCard title="Briefings" icon={ClipboardList} empty="Nenhum briefing enviado.">{data.briefings.map(item => <div key={item.id} className="flex items-center justify-between gap-4 border-t border-white/8 py-4 first:border-0"><div><p className="font-medium">{item.project_name}</p><p className="mt-1 text-xs text-white/42">Enviado em {new Date(item.created_at).toLocaleDateString("pt-BR")}</p></div><Badge>{labels[item.status] ?? item.status}</Badge></div>)}</PortalCard></div>
    </div>
  </section>
}
